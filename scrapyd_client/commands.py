...
def projects(args):
    ...
    _projects = lib.get_projects(args.target, username=args.username, password=args.password)
    if _projects:
        print('\n'.join(_projects))
...
def schedule(args):
    ...
    _projects = lib.get_projects(args.target, args.project, username=args.username, password=args.password)
    for project in _projects:
        _spiders = lib.get_spiders(args.target, project, args.spider, username=args.username, password=args.password)
        for spider in _spiders:
            job_id = lib.schedule(args.target, project, spider, job_args, username=args.username, password=args.password)
